...
//
//     * watcher .active, .pending attributes have is_ prefix (is_active, is_pending)
//
...
//
// Many thanks to Malek for his great work.
//
// Send comments and everything to Sergey Shepelev <temotor@gmail.com>.
// *************************************
...
// ev2.Error
static PyObject *pyev2_Error;

// the 'default loop'
/*@null@*/ static Loop *pyev2_DefaultLoop = NULL;
...
#define PYEV2_T_DEV_RDEV T_LONGLONG
#else
#define PYEV2_T_DEV_RDEV T_LONG
#endif // HAVE_LONG_LONG

#ifdef HAVE_LARGEFILE_SUPPORT
#define PYEV2_T_INO_SIZE T_LONGLONG
#else
#define PYEV2_T_INO_SIZE T_LONG
...
// the memory reallocator libev will use.
/*@null@*/ static void *
pyev2_realloc(void *ptr, long size)
{
    if (0 == size) {
        PyMem_Free(ptr);
        return NULL;
...
// check for a non-negative (>=0) float
static int
check_non_negative_float(const double value)
{
    if (value < 0.0) {
        PyErr_SetString(PyExc_ValueError, "A float >= 0.0 is required.");
        return -1;
    }
...
new_loop(PyTypeObject *type, unsigned int flags, int default_loop)
{
    ...
    if (!self->loop) {
        PyErr_SetString(pyev2_Error, "could not create Loop, bad 'flags'?");
        Py_DECREF(self);
        return NULL;
    }
...
Loop_dealloc(Loop *self)
{
    if (self->loop) {
        if (ev_is_default_loop(self->loop)) {
            //ev_default_destroy();
            // From ev doc: In general it is not advisable to call this function
            // except in the rare occasion where you really need to free
            // e.g. the signal handling pipe fds. If you need dynamically allocated loops 
            // it is better to use ev_loop_new and ev_loop_destroy.
            pyev2_DefaultLoop = NULL;
        }
        else {
...
Loop_new(PyTypeObject *type, PyObject *args, /*@unused@*/ PyObject *kwargs)
{
...
Loop_set_io_collect_interval(Loop *self, PyObject *args)
{
    ...
    if (check_non_negative_float(interval)) {
        return NULL;
    }
...
"verify()\n\
\n\
This method only does something with a debug build of pyev2 (which needs a debug\n\
...
static PyMethodDef Loop_methods[] = {
    ...
    {"set_io_collect_interval", (PyCFunction)Loop_set_io_collect_interval, METH_VARARGS, Loop_set_collect_interval_doc},
    {"set_timeout_collect_interval", (PyCFunction)Loop_set_timeout_collect_interval, METH_VARARGS, Loop_set_collect_interval_doc},
#if EV_VERIFY
...
static PyObject *
Loop_backend_get(Loop *self, /*@unused@*/ void *closure)
{
...
"count -> int\n\
\n\
Count of loop iterations for the loop, which is identical to the\n\
...
static PyObject *
Loop_count_get(Loop *self, /*@unused@*/ void *closure)
{
    return PyLong_FromUnsignedLong(ev_loop_count(self->loop));
}


// Loop.is_default -> bool
PyDoc_STRVAR(Loop_is_default_doc,
"True if this Loop is the 'default loop', False otherwise.");

static PyObject *
Loop_is_default_get(Loop *self, /*@unused@*/ void *closure)
{
    if (ev_is_default_loop(self->loop)) {
        Py_RETURN_TRUE;
    }
...
"now -> float\n\
\n\
Current 'event loop time', which is the time the event loop received\n\
...
static PyObject *
Loop_now_get(Loop *self, /*@unused@*/ void *closure)
{
...
static PyGetSetDef Loop_getsets[] = {
    ...
    {"count",      (getter)Loop_count_get,      NULL, Loop_count_doc,      NULL},
    {"is_default", (getter)Loop_is_default_get, NULL, Loop_is_default_doc, NULL},
    {"now",        (getter)Loop_now_get,        NULL, Loop_now_doc,        NULL},
    {NULL}  // Sentinel
};
...
// init the _Watcher - called by subtypes tp_new
static void
              ...
              int default_loop, void *cb_closure)
{
    ...
    if (!PyObject_TypeCheck(loop, &LoopType)) {
        PyErr_SetString(PyExc_TypeError, "a ev2.Loop is required");
        return -1;
    }
    else {
        if (default_loop && !ev_is_default_loop(loop->loop)) {
            PyErr_SetString(pyev2_Error, "loop must be the 'default loop'");
            return -1;
        }
    }
...
_Watcher_invoke(_Watcher *self, PyObject *args)
{
    ...
    if (!PyArg_ParseTuple(args, "i:invoke", &events)) {
        return NULL;
    }
...
{
    int revents = ev_clear_pending(self->loop->loop, self->watcher);
...
static PyMethodDef _Watcher_methods[] = {
    {"invoke", (PyCFunction)_Watcher_invoke, METH_VARARGS, _Watcher_invoke_doc},
    {"clear_pending", (PyCFunction)_Watcher_clear_pending, METH_NOARGS, _Watcher_clear_pending_doc},
    {NULL}  // Sentinel
};
...
static PyMemberDef _Watcher_members[] = {
    {"loop", T_OBJECT_EX, offsetof(_Watcher, loop), READONLY,
     "ev2.Loop object to which this watcher is attached"},
...
static PyObject *
_Watcher_callback_get(_Watcher *self, /*@unused@*/ void *closure)
{
...
static PyObject *
_Watcher_is_active_get(_Watcher *self, /*@unused@*/ void *closure)
{
    if (ev_is_active(self->watcher)) {
        Py_RETURN_TRUE;
    }
...
static PyObject *
_Watcher_is_pending_get(_Watcher *self, /*@unused@*/ void *closure)
{
...
static PyObject *
_Watcher_priority_get(_Watcher *self, /*@unused@*/ void *closure)
{
    return PyLong_FromLong(ev_priority(self->watcher));
}

static int
_Watcher_priority_set(_Watcher *self, PyObject *value, /*@unused@*/ void *closure)
{
    if (ev_is_active(self->watcher) || ev_is_pending(self->watcher)) {
        PyErr_SetString(pyev2_Error, "you cannot change the 'priority' of a "
    ...
    {
        long new_priority = PyLong_AsLong(value);
        if (PyErr_Occurred())
            return -1;
        ev_set_priority(self->watcher, (int)new_priority);
    }
...
static PyGetSetDef _Watcher_getsets[] = {
    {"callback",   (getter)_Watcher_callback_get,   (setter)_Watcher_callback_set, _Watcher_callback_doc, NULL},
    {"is_active",  (getter)_Watcher_is_active_get,  NULL, _Watcher_is_active_doc, NULL},
    {"is_pending", (getter)_Watcher_is_pending_get, NULL, _Watcher_is_pending_doc, NULL},
    {"priority",   (getter)_Watcher_priority_get,   (setter)_Watcher_priority_set, _Watcher_priority_doc, NULL},
    {NULL}  // Sentinel
};
...
'loop': a ev2.Loop object to which the watcher will be attached.\n\
...
set_io(Io *self, PyObject *fd, unsigned long events)
{
    if (events & ~(EV_READ | EV_WRITE)) {
        PyErr_SetString(pyev2_Error, "illegal event mask");
        return -1;
    }
...
static PyMethodDef Io_methods[] = {
    ...
    {"start", (PyCFunction)Io_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop",  (PyCFunction)Io_stop,  METH_NOARGS,  _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
{
    if (check_non_negative_float(repeat)) {
        return -1;
    }
...
static PyMethodDef Timer_methods[] = {
    ...
    {"start", (PyCFunction)Timer_start, METH_NOARGS, _Watcher_start_doc},
    {"stop", (PyCFunction)Timer_stop, METH_NOARGS, _Watcher_stop_doc},
    {"again", (PyCFunction)Timer_again, METH_NOARGS, Timer_again_doc},
    {NULL}  // Sentinel
};
...
static PyObject *
Timer_repeat_get(Timer *self, /*@unused@*/ void *closure)
{
    return PyFloat_FromDouble(self->timer.repeat);
}

static int
Timer_repeat_set(Timer *self, PyObject *value, /*@unused@*/ void *closure)
{
    double repeat;
...
static PyGetSetDef Timer_getsets[] = {
    {"repeat", (getter)Timer_repeat_get, (setter)Timer_repeat_set, Timer_repeat_doc, NULL},
    {NULL}  // Sentinel
};
...
error:
    ...
    PyErr_Format(pyev2_Error, "due to previous error, "
                 "<ev2.Periodic object at %p> will be stopped", periodic);
    PyErr_WriteUnraisable(periodic->reschedule_cb);
...
static PyMethodDef Periodic_methods[] = {
    ...
    {"start", (PyCFunction)Periodic_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop", (PyCFunction)Periodic_stop,   METH_NOARGS,  _Watcher_stop_doc},
    {"again", (PyCFunction)Periodic_again, METH_VARARGS, Periodic_again_doc},
    {"at", (PyCFunction)Periodic_at,       METH_VARARGS, Periodic_at_doc},
    {NULL}  // Sentinel
};
...
static PyObject *
Periodic_interval_get(Periodic *self, /*@unused@*/ void *closure)
{
    return PyFloat_FromDouble(self->periodic.interval);
}

static int
Periodic_interval_set(Periodic *self, PyObject *value, /*@unused@*/ void *closure)
{
    double interval;

    if (!value) {
        PyErr_SetString(PyExc_TypeError, "Cannot delete attribute 'interval'.");
        return -1;
    }
...
Periodic_reschedule_cb_set(Periodic *self, PyObject *value, void *closure)
{
    ...
    if (!value) {
        PyErr_SetString(PyExc_TypeError, "Cannot delete attribute 'reschedule_cb'.");
        return -1;
    }
...
static PyGetSetDef Periodic_getsets[] = {
    {"interval", (getter)Periodic_interval_get, (setter)Periodic_interval_set, Periodic_interval_doc, NULL},
    {"reschedule_cb", (getter)Periodic_reschedule_cb_get, (setter)Periodic_reschedule_cb_set, Periodic_reschedule_cb_doc, NULL},
    {NULL}  // Sentinel
};
...
{
    if (signum <= 0) {
        PyErr_SetString(pyev2_Error, "Illegal signal number.");
        return -1;
    }
...
static PyMethodDef Signal_methods[] = {
    ...
    {"start", (PyCFunction)Signal_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop",  (PyCFunction)Signal_stop,  METH_NOARGS,  _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
static PyMethodDef Child_methods[] = {
    ...
    {"start", (PyCFunction)Child_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop",  (PyCFunction)Child_stop,  METH_NOARGS,  _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
// new_statdata - creates a Statdata
/*@null@*/ static Statdata *
...
static PyMemberDef Statdata_members[] = {
    ...
    {"dev", PYEV2_T_DEV_RDEV, offsetof(Statdata, statdata.st_dev), READONLY,
     "device"},
    {"rdev", PYEV2_T_DEV_RDEV, offsetof(Statdata, statdata.st_rdev), READONLY,
     "device type"},
    {"ino", PYEV2_T_INO_SIZE, offsetof(Statdata, statdata.st_ino), READONLY,
     "inode"},
    {"size", PYEV2_T_INO_SIZE, offsetof(Statdata, statdata.st_size), READONLY,
     "total size, in bytes"},
    {NULL}  // Sentinel
};
...
Stat_stat(Stat *self)
{
    ...
    if (self->stat.attr.st_nlink == 0) {
        return PyErr_SetFromErrnoWithFilename(PyExc_OSError, (char*)self->stat.path);
    }
...
static PyMethodDef Stat_methods[] = {
    ...
    {"start", (PyCFunction)Stat_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop",  (PyCFunction)Stat_stop,  METH_NOARGS,  _Watcher_stop_doc},
...
static PyMethodDef Idle_methods[] = {
    {"start", (PyCFunction)Idle_start, METH_NOARGS, _Watcher_start_doc},
    {"stop", (PyCFunction)Idle_stop, METH_NOARGS, _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
static PyMethodDef Prepare_methods[] = {
    {"start", (PyCFunction)Prepare_start, METH_NOARGS, _Watcher_start_doc},
    {"stop", (PyCFunction)Prepare_stop, METH_NOARGS, _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
static PyMethodDef Check_methods[] = {
    {"start", (PyCFunction)Check_start, METH_NOARGS, _Watcher_start_doc},
    {"stop", (PyCFunction)Check_stop, METH_NOARGS, _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
'other': a ev2.Loop to embed, the given 'other' must be embeddable (i.e. its\n\
...
set_embed(Embed *self, Loop *other)
{
    ...
    if (!(ev_backend(other->loop) & ev_embeddable_backends())) {
        PyErr_SetString(pyev2_Error, "'other' must be embeddable");
        return -1;
    }
...
static PyMethodDef Embed_methods[] = {
    ...
    {"start", (PyCFunction)Embed_start, METH_NOARGS,  _Watcher_start_doc},
    {"stop",  (PyCFunction)Embed_stop,  METH_NOARGS,  _Watcher_stop_doc},
...
static PyGetSetDef Embed_getsets[] = {
    {"callback", (getter)_Watcher_callback_get, (setter)_Watcher_callback_set, _Watcher_callback_doc, (void *)1},
    {NULL}  // Sentinel
};
...
static PyMethodDef Fork_methods[] = {
    {"start", (PyCFunction)Fork_start, METH_NOARGS, _Watcher_start_doc},
    {"stop",  (PyCFunction)Fork_stop,  METH_NOARGS, _Watcher_stop_doc},
    {NULL}  // Sentinel
};
...
static PyMethodDef Async_methods[] = {
    {"start", (PyCFunction)Async_start, METH_NOARGS, _Watcher_start_doc},
    {"stop", (PyCFunction)Async_stop, METH_NOARGS, _Watcher_stop_doc},
...
// ev2.__doc__
PyDoc_STRVAR(pyev2_doc, "\
pyev2 is an extension wrapper around libev.\n\
\n\
See __version__ tuple for pyev2 version.\n\
See libev_version tuple for libev headers version this module was compiled against.\n\
See abi_version tuple for libev ABI version.\n\
*WARNING*: Mismatch in major versions of libev_version and abi_version most likely will cause unexpected errors.\n\
You are encouraged to check libev and abi versions.\n\
\n\
...
\n\
Libev is written and maintained by Marc Lehmann. The main documentation for\n\
libev is at: http://pod.tst.eu/http://cvs.schmorp.de/libev/ev.pod\n\
");


// ev2.time() -> float
PyDoc_STRVAR(pyev2_time_doc,
"time() -> float\n\
\n\
...
/*@null@*/ static PyObject *
pyev2_sleep(PyObject *module, PyObject *args)
{
    double interval;
...
// ev2.supported_backends() -> int
PyDoc_STRVAR(pyev2_supported_backends_doc,
...
static PyObject *
pyev2_supported_backends(PyObject *module)
{
    return PyLong_FromUnsignedLong(ev_supported_backends());
}


// ev2.recommended_backends() -> int
PyDoc_STRVAR(pyev2_recommended_backends_doc,
...
static PyObject *
pyev2_recommended_backends(PyObject *module)
{
    return PyLong_FromUnsignedLong(ev_recommended_backends());
}


// ev2.embeddable_backends() -> int
PyDoc_STRVAR(pyev2_embeddable_backends_doc,
...
static PyObject *
pyev2_embeddable_backends(PyObject *module)
{
    return PyLong_FromUnsignedLong(ev_embeddable_backends());
}


// ev2.default_loop([flags]) -> Loop
PyDoc_STRVAR(pyev2_default_loop_doc,
...
for your application you can either create a Loop that doesn't do that, or\n\
...
/*@null@*/ static PyObject *
pyev2_default_loop(PyObject *module, PyObject *args)
{
    unsigned int flags = EVFLAG_AUTO;
    ...
    if (!pyev2_DefaultLoop) {
        pyev2_DefaultLoop = new_loop(&LoopType, flags, 1);
        if (!pyev2_DefaultLoop) {
            return NULL;
        }
    }
        ...
        Py_INCREF(pyev2_DefaultLoop);
    }

    return (PyObject *)pyev2_DefaultLoop;
}


// ev2.destroy_default_loop() -> None
PyDoc_STRVAR(pyev2_destroy_default_loop_doc,
"destroy_default_loop() -> None\n\
\n\
This will destroy the 'default loop'.\n\
\n\
From libev documentation: In general it is not advisable to call this function\n\
    except in the rare occasion where you really need to free\n\
    e.g. the signal handling pipe fds. If you need dynamically allocated loops\n\
    it is better to use ev_loop_new and ev_loop_destroy.\n\
\n\
WARNING: if you stored references to .default_loop() result,\n\
using those will likely crash your program. I.e. code:\n\
>>> DL = ev2.default_loop()\n\
>>> ev2.destroy_default_loop()\n\
>>> DL.loop() # this will crash. You must renew DL name with DL = ev2.default_loop().\n\
\n\
If you didn't store references to default loop, i.e. code:\n\
>>> print ev2.default_loop().now\n\
>>> ev2.destroy_default_loop()\n\
>>> ev2.default_loop().loop()\n\
then, you have nothing to worry about.\n\
\n\
If you want to recreate the loop, just call .default_loop() as you normally would.\n\
");

static PyObject *
pyev2_destroy_default_loop(PyObject *module, PyObject *args)
{
    ev_default_destroy();
    pyev2_DefaultLoop = NULL;
...
static PyMethodDef module_methods[] = {
    {"time", (PyCFunction)pyev2_time, METH_NOARGS, pyev2_time_doc},
    {"sleep", (PyCFunction)pyev2_sleep, METH_VARARGS, pyev2_sleep_doc},
    {"supported_backends", (PyCFunction)pyev2_supported_backends, METH_NOARGS, pyev2_supported_backends_doc},
    {"recommended_backends", (PyCFunction)pyev2_recommended_backends, METH_NOARGS, pyev2_recommended_backends_doc},
    {"embeddable_backends", (PyCFunction)pyev2_embeddable_backends, METH_NOARGS, pyev2_embeddable_backends_doc},
    {"destroy_default_loop", (PyCFunction)pyev2_destroy_default_loop, METH_NOARGS, pyev2_destroy_default_loop_doc},
    {"default_loop", (PyCFunction)pyev2_default_loop, METH_VARARGS, pyev2_default_loop_doc},
    {NULL} // Sentinel
};

PyMODINIT_FUNC
init_ev2(void) {
    PyObject *version = NULL, *libev_version = NULL, *abi_version = NULL;

    // fill in deferred fields
    ...
    // init module
    module = Py_InitModule3("_ev2", module_methods, pyev2_doc);
    if (NULL == module)
        return;

    // ev2.__version__
    version = Py_BuildValue("(ii)", PYEV2_VERSION_MAJOR, PYEV2_VERSION_MINOR);
    if (!version) {
        return;
    }

    // ev2.libev_version
    libev_version = Py_BuildValue("(ii)", EV_VERSION_MAJOR, EV_VERSION_MINOR);
    if (!libev_version) {
        Py_DECREF(version);
        return;
    }

    abi_version = Py_BuildValue("(ii)", ev_version_major(), ev_version_minor());
    if (!abi_version) {
        Py_DECREF(version);
        Py_DECREF(libev_version);
        return;
    }

    // ev2.Error object
    pyev2_Error = PyErr_NewException("_ev2.Error", NULL, NULL);
    if (!pyev2_Error) {
        Py_DECREF(version);
        Py_DECREF(libev_version);
        Py_DECREF(abi_version);
        return;
    }
    ...
    if (
        ...
        PyModule_AddObject(module, "libev_version", libev_version) ||
        PyModule_AddObject(module, "abi_version", abi_version) ||
        PyModule_AddObject(module, "Error", pyev2_Error) ||

        // types
        PyModule_AddObject(module, "Loop", (PyObject *)&LoopType) ||
        PyModule_AddObject(module, "Io", (PyObject *)&IoType) ||
        PyModule_AddObject(module, "Timer", (PyObject *)&TimerType) ||
        PyModule_AddObject(module, "Periodic", (PyObject *)&PeriodicType) ||
        PyModule_AddObject(module, "Signal", (PyObject *)&SignalType) ||
        PyModule_AddObject(module, "Child", (PyObject *)&ChildType) ||
        PyModule_AddObject(module, "Stat", (PyObject *)&StatType) ||
        PyModule_AddObject(module, "Idle", (PyObject *)&IdleType) ||
        PyModule_AddObject(module, "Prepare", (PyObject *)&PrepareType) ||
        PyModule_AddObject(module, "Check", (PyObject *)&CheckType) ||
        PyModule_AddObject(module, "Embed", (PyObject *)&EmbedType) ||
        PyModule_AddObject(module, "Fork", (PyObject *)&ForkType) ||
        PyModule_AddObject(module, "Async", (PyObject *)&AsyncType) ||

        // Loop() and default_loop() flags and backends
        PyModule_AddUnsignedIntMacro(module, EVFLAG_AUTO) ||
        PyModule_AddUnsignedIntMacro(module, EVFLAG_NOENV) ||
        PyModule_AddUnsignedIntMacro(module, EVFLAG_FORKCHECK) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_SELECT) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_POLL) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_EPOLL) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_KQUEUE) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_DEVPOLL) ||
        PyModule_AddUnsignedIntMacro(module, EVBACKEND_PORT) ||

        // Loop.loop() flags
        PyModule_AddIntMacro(module, EVLOOP_NONBLOCK) ||
        PyModule_AddIntMacro(module, EVLOOP_ONESHOT) ||

        // Loop.unloop() how
        PyModule_AddIntMacro(module, EVUNLOOP_ONE) ||
        PyModule_AddIntMacro(module, EVUNLOOP_ALL) ||

        // priorities
        PyModule_AddIntMacro(module, EV_MINPRI) ||
        PyModule_AddIntMacro(module, EV_MAXPRI) ||

        // events
        PyModule_AddUnsignedIntMacro(module, EV_READ) ||
        PyModule_AddUnsignedIntMacro(module, EV_WRITE) ||
        PyModule_AddUnsignedIntMacro(module, EV_TIMEOUT) ||
        PyModule_AddUnsignedIntMacro(module, EV_PERIODIC) ||
        PyModule_AddUnsignedIntMacro(module, EV_SIGNAL) ||
        PyModule_AddUnsignedIntMacro(module, EV_CHILD) ||
        PyModule_AddUnsignedIntMacro(module, EV_STAT) ||
        PyModule_AddUnsignedIntMacro(module, EV_IDLE) ||
        PyModule_AddUnsignedIntMacro(module, EV_PREPARE) ||
        PyModule_AddUnsignedIntMacro(module, EV_CHECK) ||
        PyModule_AddUnsignedIntMacro(module, EV_EMBED) ||
        PyModule_AddUnsignedIntMacro(module, EV_FORK) ||
        PyModule_AddUnsignedIntMacro(module, EV_ASYNC) ||
        PyModule_AddUnsignedIntMacro(module, EV_ERROR)
       ) {
        Py_DECREF(version);
        Py_DECREF(pyev2_Error);
        return;
    }

    // setup libev
    ev_set_allocator(pyev2_realloc);
